...
import com.sc.dto.ImageDto;
import com.sc.error.ServerErrorWithString;
...
public class ImageModule {
    ...
    public Response uploadImage(@FormDataParam("file") InputStream uploadedInputStream,
        ...
        if (fileNameSplits.length < 2 ||
                !appendix.equalsIgnoreCase("jpg") &&
                        !appendix.equalsIgnoreCase("png") &&
                        !appendix.equalsIgnoreCase("jpeg")) {
        ...
        try {
            AliyunOSSImpl oss = GuiceInstance.getGuiceInjector().getInstance(AliyunOSSImpl.class);
            String storeETag = oss.uploadFile(storedFileName, uploadedInputStream);
